Projet robotique sur Webots — TP2
=================================

API du contrôleur Java
----------------------
...
-  `0. Informations et objectifs <#0-informations-et-objectifs>`__
   ...
   -  `0.2 Matériel et outils
      nécessaires <#02-matériel-et-outils-nécessaires>`__
   -  `0.3 Objectifs de la séance <#03-objectifs-de-la-séance>`__

-  `1. Organisation du contrôleur <#1-organisation-du-contrôleur>`__

   -  `1.1 Conception du contrôleur <#11-conception-du-contrôleur>`__
   -  `1.2 Organisation des fichiers <#12-organisation-des-fichiers>`__
   -  `1.3 Point d’entrée du
      contrôleur <#13-point-dentrée-du-contrôleur>`__
   -  `1.4 Rôle de la classe Robot <#14-rôle-de-la-classe-robot>`__
   -  `1.5 Travail à réaliser <#15-travail-à-réaliser>`__

-  `2. Complétion de l’API moteurs <#2-complétion-de-lapi-moteurs>`__
-  `3. Complétion de l’API capteurs <#3-complétion-de-lapi-capteurs>`__
-  `4. Complétion des APIs du bras et de la
   pince <#4-complétion-des-apis-du-bras-et-de-la-pince>`__
-  `5. Comportement du robot <#5-comportement-du-robot>`__
...
Dans le **TP1**, vous avez finalisé la structure du robot directement
dans Webots.
...
Dans ce **TP2**, vous allez travailler davantage sur le **code Java du
contrôleur**.

L’objectif n’est pas encore de programmer la récupération complète des
palets.
...
-  `Documentation
   Webots <https://cyberbotics.com/doc/reference/index>`__
...
Pour faciliter la compréhension et l’évolution du projet, le contrôleur
du robot a été divisé en plusieurs classes formant une petite API.

Cette API permet d’utiliser le robot plus simplement, sans manipuler
directement tous les moteurs et capteurs dans le programme principal.
...
.. figure:: ../images/tp2/tp2_architecture_controleur.png
   :alt: Architecture générale du contrôleur Java
...
Par exemple, au lieu de contrôler chaque roue séparément, on pourra
simplement écrire :
...
Ce nom doit correspondre au nom indiqué dans le champ ``controller`` du
robot dans Webots.
...
Dans ce TP, le comportement utilisé est un comportement simple
d’évitement d’obstacles.

.. figure:: ../images/tp2/tp2_demarrage_controleur.png
   :alt: Démarrage du contrôleur
...
Le ``Supervisor`` est un objet Webots qui permet au contrôleur de
...
La classe ``Robot`` récupère ensuite le ``timeStep``, qui correspond au
pas de temps utilisé par Webots.
...
#. Webots avance la simulation
#. le scheduler est mis à jour
#. le comportement du robot est exécuté

C’est donc dans la méthode ``update()`` du comportement que seront prises
les décisions du robot.

.. figure:: ../images/tp2/tp2_role_robot.png
   :alt: Rôle de la classe Robot
...
Une grande partie du projet est déjà fournie afin de conserver une
structure propre et fonctionnelle.

Votre objectif est de compléter les méthodes manquantes dans les fichiers
indiqués, puis de vérifier que le comportement simple du robot fonctionne
correctement.
...
.. figure:: ../images/tp2/tp2_api_moteurs.png
   :alt: Organisation de l’API moteurs
...
Elle encapsule un moteur Webots de type ``Motor`` et fournit des méthodes
simples :
...
.. figure:: ../images/tp2/tp2_wheel_motor.png
   :alt: Roue et moteur Webots
...
Au lieu de manipuler directement un moteur Webots, on utilise une classe
plus simple.
...
Dans le constructeur, le moteur doit être configuré en rotation continue
avec :
...
#. mémoriser la vitesse courante
#. appliquer cette vitesse au moteur Webots


TODO 1.3 — Avancer
""""""""""""""""""

La méthode ``forward(double speed)`` doit faire avancer la roue avec une
vitesse positive.
...
La méthode ``backward(double speed)`` doit faire reculer la roue avec une
vitesse négative.
...
La classe ``MotorGroup`` permet d’appliquer la même commande à plusieurs
roues.
...
La méthode ``setSpeed(double speed)`` doit appliquer la vitesse à toutes
les roues du groupe.
...
Elle regroupe les quatre roues et fournit des méthodes de déplacement
plus lisibles.
...
.. figure:: ../images/tp2/tp2_mouvements_drivebase.png
   :alt: Mouvements de la base roulante
...
L’API capteurs permet de lire plus facilement les valeurs des capteurs
Webots.

Dans ce TP, vous allez utiliser principalement les capteurs de distance.

.. figure:: ../images/tp2/tp2_capteurs.png
   :alt: Capteurs du robot
...
La classe ``DistanceSensorWrapper`` encapsule un capteur de distance
Webots.
...
La méthode ``detectsObject(double threshold)`` doit retourner ``true`` si
la valeur du capteur est supérieure au seuil donné.
...
.. figure:: ../images/tp2/tp2_sensor_manager.png
   :alt: Rôle de SensorManager
...
Vous n’avez pas besoin de le compléter, mais vous devez le lire afin de
comprendre comment les différentes parties de l’API sont utilisées
ensemble.
...
#. avance si aucun obstacle n’est présent
#. tourne si un obstacle est détecté
...
Dans ce TP, vous avez commencé à travailler sur le contrôleur Java du
robot.

L’objectif principal était de comprendre l’organisation du code et de
compléter une API simple permettant de piloter les différentes parties
du robot.

Vous avez vu que le contrôleur est divisé en plusieurs classes afin de
rendre le code plus clair et plus facile à réutiliser.

Cette API permet de commander le robot avec des méthodes simples, par
exemple pour :
...
Vous avez également compris le rôle de la classe ``Robot``, qui sert de
lien entre Webots, les différentes API du robot et le comportement à
exécuter.
...
Le comportement utilisé, ``SimpleAvoidObstacleBehavior``, était déjà
fourni.

Il permet de montrer comment les API sont utilisées ensemble : le robot
lit ses capteurs, décide s’il y a un obstacle, puis commande ses moteurs
pour avancer ou tourner.
...
-  comprendre le lien entre ``FourWheelsCollisionAvoidanceAPI``,
   ``Robot`` et le comportement du robot
...
Les fonctions complétées ici seront réutilisées pour créer des
comportements plus complexes, notamment :
