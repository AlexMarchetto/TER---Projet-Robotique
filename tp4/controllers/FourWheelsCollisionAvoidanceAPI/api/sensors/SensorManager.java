...
public class SensorManager {
  ...
  public SensorManager(Supervisor robot, int timeStep) {
    // Initialize distance sensors using their Webots device names.
    ...
    // Initialize the camera used as a simple color sensor.
    this.colorSensor = new ColorSensorWrapper(
        robot.getCamera("color_sensor"),
        timeStep
    );

    // Initialize the front touch sensor used to detect physical contact.
  ...
  public void update() {
    // Update sensors that need to track their previous and current state.
