...
public class PuckManager {
  ...
  public Node getPuckNode(int index) { return (index < 0 || index >= puckNodes.length) ? null : puckNodes[index]; }
  ...
  public boolean isDelivered(int index) { return (index < 0 || index >= puckDelivered.length) || puckDelivered[index]; }
  public String getPuckName(int index) { return (index < 0 || index >= puckNames.length) ? "unknown" : puckNames[index]; }
  public void attachPuckToRobot(int index) {
    ...
    double localX = 0.15, localY = 0.0, localZ = 0.09;
    double worldX = robotPosition[0] + orientation[0] * localX + orientation[1] * localY + orientation[2] * localZ;
    double worldY = robotPosition[1] + orientation[3] * localX + orientation[4] * localY + orientation[5] * localZ;
    double worldZ = robotPosition[2] + orientation[6] * localX + orientation[7] * localY + orientation[8] * localZ;
  ...
  public void dropPuck(int index, double dropX, double dropY, double dropZ) {
    ...
    if (puckTranslationFields[index] != null) { puckTranslationFields[index].setSFVec3f(new double[] {dropX, dropY, dropZ}); }
    if (puckNodes[index] != null) { puckNodes[index].resetPhysics(); }
